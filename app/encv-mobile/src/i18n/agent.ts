export default {
  "zh-CN": {
    ...
    "agent.apiKeyStatusDecryptFailedEmpty":
      "存储的 API Key 无法解密（所有格式尝试均失败）。加密密钥可能已轮换，或存储值已损坏。系统已自动清除旧值，请重新输入后按回车保存。",
    ...
    "agent.v2Scenarios.hint":
      "点击剧本后会自动切到「内置剧本」mock 模式，并发送 trigger 关键词。所有剧本都通过真实 ToolRegistry 派发，会执行 search_files / command_run / batch_rename 等真实工具调用。",
  ...
  en: {
    ...
    "agent.apiKeyStatusDecryptFailedEmpty":
      "Stored API key cannot be decrypted (all formats exhausted). The encryption key may have rotated, or the stored value is corrupted. The old value has been auto-cleared — please re-enter your key and press Enter to save.",
    ...
    "agent.apiKeyMaskHint":
      "A previously-encrypted API Key is stored here but cannot be decrypted. Re-enter and save to overwrite the corrupted value.",
    "agent.apiKeyAutoReset":
      "Old API key was unreadable and has been auto-cleared. Please re-enter your key below and press Enter to save.",
    ...
    "agent.modelErrorNoApiKey":
      "API Key not configured: the model list requires a valid OpenAI API Key. Fill in the API Key above and save.",
    "agent.modelErrorDecryptFailed":
      "Stored API Key cannot be decrypted: the encryption key may have rotated or the stored value is corrupted. Re-enter the API Key above and save to overwrite.",
