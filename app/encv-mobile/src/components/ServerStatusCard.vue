<!--
  ServerStatusCard.vue — 后端状态卡片（🆕 2026-06-15）

  设计目标：
    - 复用：Settings / ServerSettings / ServerSettingsDetail / ServerDetail / DevLogs 5 个入口
    - 信息密度：状态 + version + instance_id + port + latency + transport + 上次检查
    - 动态效果：
        ① 状态点 pulse（绿色 = 在线 / 黄色 = 探测中 / 红色 = 离线）
        ② 状态过渡（背景/边框颜色 transition 0.3s）
        ③ instance_id 变化 → 高亮闪烁 1.5s（performPingCheck 模式）
        ④ latency 数字滚动 transition
        ⑤ 上次检查时间每 30s 自动刷新（不重 mount）
