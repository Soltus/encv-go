...
export function buildDynamicWorkflowPure(
  ...
  for (const plugin of plugins) {
    ...
    for (const version of versions) {
      const encryptSelectFields = allSelectFields.filter(sf => !sf.field.condition || sf.field.condition === "encrypt");
      const encryptBoolFields = allBoolFields.filter(bf => !bf.field.condition || bf.field.condition === "encrypt");
      const decryptSelectFields = allSelectFields.filter(sf => !sf.field.condition || sf.field.condition === "decrypt");
      const decryptBoolFields = allBoolFields.filter(bf => !bf.field.condition || bf.field.condition === "decrypt");
