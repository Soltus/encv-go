...
describe('tokenizeQuery - 基础场景', () => {
  ...
  it('TestTQ_WordWithSpecialChars: word 含 - _ . 等合法字符', () => {
    expect(tokenizeQuery('my-file_v2.tar.gz')).toEqual([
      { kind: 'word', text: 'my-file_v2.tar.gz' },
    ])
  })
})
...
describe('tokenizeQuery - phrase 场景', () => {
  ...
  it('TestTQ_PhraseCJK: 中文短语', () => {
    expect(tokenizeQuery('"在线播放"')).toEqual([
      { kind: 'phrase', text: '在线播放' },
    ])
  })
  ...
  it('TestTQ_PhraseUnterminated: 未闭合 "（容错：作 word 处理）', () => {
    ...
    // 这是 UI 容错，不应报错
    expect(tokenizeQuery('"unterminated')).toEqual([
      { kind: 'word', text: '"unterminated' },
    ])
  })
...
describe('tokenizeQuery - regex 场景', () => {
  it('TestTQ_RegexSlashForm: regex:/^p/', () => {
    expect(tokenizeQuery('regex:/^test/')).toEqual([
  ...
  it('TestTQ_RegexUnterminatedSlash: regex:/^abc（缺 /）→ fallback to word', () => {
    ...
    expect(tokenizeQuery('regex:/^abc')).toEqual([
      { kind: 'word', text: 'regex:/^abc' },
    ])
  })

  it('TestTQ_RegexEmpty: regex: 后无内容 → word', () => {
    // pattern 为空时不切 regex
    expect(tokenizeQuery('regex:')).toEqual([
...
describe('tokenizeQuery - 布尔操作符', () => {
  ...
  it('TestTQ_OpPartOfWord: "ANDROID" 应作 word 不作 op', () => {
    ...
    expect(tokenizeQuery('ANDROID')).toEqual([
      { kind: 'word', text: 'ANDROID' },
    ])
  })

  it('TestTQ_OpPartOfWord2: "ANDROID-12" 应作 word', () => {
    expect(tokenizeQuery('ANDROID-12')).toEqual([
      { kind: 'word', text: 'ANDROID-12' },
    ])
  })
...
describe('renderSnippet - 真实 snippet 场景（来自 FTS5）', () => {
  ...
  it('TestRS_NoMatchHighlight: snippet 不含高亮时', () => {
    // 部分 FTS5 实现可能不返回高亮
    expect(renderSnippet('普通文本 没有高亮')).toEqual([
      { text: '普通文本 没有高亮', highlight: false },
    ])
  })
})
