...
vi.mock('@ionic/vue', () => ({
  ...
  IonBackButton: defineComponent({ name: 'IonBackButton', props: ['defaultHref', 'text'], template: '<button class="ion-back-button-stub"><slot /></button>' }),
  ...
  IonSegment: defineComponent({ name: 'IonSegment', props: ['modelValue', 'mode'], emits: ['update:modelValue'], template: '<div class="ion-segment-stub"><slot /></div>' }),
  IonSegmentButton: defineComponent({ name: 'IonSegmentButton', props: ['value'], template: '<button class="ion-segment-button-stub"><slot /></button>' }),
  ...
  IonModal: defineComponent({ name: 'IonModal', props: ['isOpen'], emits: ['didDismiss'], template: '<div class="ion-modal-stub" v-if="isOpen"><slot /></div>' }),
...
vi.mock('@/components/group-detail/PipelineTab.vue', () => ({
  default: defineComponent({ name: 'PipelineTab', props: ['run', 'jobs', 'total', 'passed', 'failed', 'pending', 'skipped'], template: '<div class="pipeline-tab-stub" />' }),
}))
vi.mock('@/components/group-detail/TasksTab.vue', () => ({
  default: defineComponent({
    ...
    template: '<div class="tasks-tab-stub"><div v-for="tk in runTasks" :key="tk.id" class="gd-task-row" :data-task-id="tk.id">{{ tk.id }}</div></div>',
  }),
}))
...
vi.mock('@/components/group-detail/FilterDrawer.vue', () => ({
  default: defineComponent({ name: 'FilterDrawer', props: ['status', 'taskType', 'plugin', 'availablePlugins'], emits: ['update:status', 'update:task-type', 'update:plugin', 'reset', 'apply'], template: '<div class="filter-drawer-stub" />' }),
}))
 ...
 */
async function mountGroupDetail(opts: {
  runId?: string
  tasksFixture?: EncvTask[]
  summaryFixture?: RunSummary
} = {}) {
...
describe('GroupDetail.vue 真机组件测试', () => {
  ...
  it('totals 从后端 SQL summary 拿（不靠 store.tasks 算）', async () => {
    ...
    const tasksFixture = [
      makeTask('t-1', RUN_ID, 'completed'),
      makeTask('t-2', RUN_ID, 'completed'),
      makeTask('t-3', RUN_ID, 'failed'),
    ]
