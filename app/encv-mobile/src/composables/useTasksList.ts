...
function createUseTasksList() {
  ...
  async function fetchTasks(_opts?: { silent?: boolean }): Promise<void> {
    ...
    try {
      ...
      // 🆕 2026-06-22 v2 架构：rebuildFromBackend（全量替换，不增量 patch）
      //   历史：旧 reconcileWithBackend 增量逻辑 + bulkSet 双路径
      //   → 1000+ 任务时多次 O(n) patch 累积 O(n²) 卡顿
      //   → 增量 patch 易遗漏状态（如 createdAt 不一致）
      //   改：直接以 serverTasks 全量替换 store（一次 O(n) 完成）
