    ...
    jobs: [
      {
        ...
        jobDefId: moduleId,
        status: moduleStatusToWorkflowStatus(state.status),
        conclusion: state.status === 'done' ? 'success'
...
export function useWebDavAutomationTests(): UseWebDavAutomationTestsReturn {
  ...
  function clearHistory(): void {
    ...
    const remaining = service.runs.value.filter(
      (r) => r.workflowRun?.workflowDefId !== WEBDAV_WORKFLOW_DEF_ID,
    )
