...
import type { UnifiedRunRecord, WorkflowDefinition } from "@/lib/workflow/types";
...
import { __resetServiceForTests, useWorkflowTaskService } from "@/composables/useWorkflowTaskService";
...
describe("useWorkflowTaskService — submitRun 基本流程", () => {
  ...
  it("submitRun 拒绝重复运行（isRunning 时抛错）", async () => {
    ...
    await expect(service.submitRun({ workflow: makeSimpleWorkflow() })).rejects.toThrow("A workflow is already running");
  });
});
