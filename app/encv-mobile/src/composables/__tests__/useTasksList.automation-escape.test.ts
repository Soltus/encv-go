...
function buildDynamicSteps(plugins: PluginMeta[], mockRoot: string): any[] {
  ...
  for (const plugin of plugins) {
    ...
    const sourcePath = specRelPath
      ...
      : `${mockRoot}01-plain-media/misc/sample.${sourceExt}`
    ...
    for (const version of versions) {
      ...
      for (const sel of encSelectCombos) {
        for (const bool of encBoolCombos) {
          ...
          enc.forEach((sf, i) => { if (sel[i] !== undefined) extraFields[sf.field.key] = sel[i] })
          encBool.forEach((bf, i) => { extraFields[bf.field.key] = bool[i] ? 'true' : 'false' })
          steps.push({ id: `enc_${plugin.name}_v${version}_${sourceExt}_${JSON.stringify(extraFields)}`,
            pluginName: plugin.name, taskType: 'encrypt', sourcePath, sourceExt, version, extraFields })
        }
      }
    }
  }
...
function dumpFakeGroupings(wrapper: VueWrapper, label: string, max = 20): void {
  ...
  // eslint-disable-next-line no-console
  console.log(`[UI-LOGIC] ${label}: ${fake.length} 个伪 group（__manual__）：前 ${Math.min(max, fake.length)} = [${fake.slice(0, max).join(', ')}]`)
}
...
describe('真机"逃逸"渲染逻辑层面诊断（非样式）', () => {
  ...
  it('L1: 1+1+1000 规模 + 完整逃逸链路 + viewMode / sortOrder / filter 切换', async () => {
    ...
    for (const run of RUNS) {
      for (let i = 0; i < run.stepCount; i++) {
        ...
        store.appendTask(makeTask(`t-${idx++}`, {
          runId: run.id, triggeredBy: 'automation', status: 'running',
          createdAt: run.createdAt, pluginName: s.pluginName,
        }))
      }
    }
    // eslint-disable-next-line no-console
    ...
    // eslint-disable-next-line no-console
    console.log(`[UI-LOGIC] L1-2 关键诊断：flat 模式 displayedItems=${wrapper.findAll('[data-testid^="item-"]').length}（应该=1002，flat 无 group 概念）`)
    ...
    for (let i = 0; i < idx; i++) {
      ...
      fetchTasksData.push(
        Math.random() < 0.5 ? { ...t, runId: undefined } : t,
      )
    }
  ...
  it('L2: 时间桶切换（today / yesterday / thisWeek / thisMonth / earlier）对逃逸的影响', async () => {
    ...
    for (const bucket of buckets) {
      for (let i = 0; i < 5; i++) {
        store.appendTask(makeTask(`t-${idx++}`, {
          runId: `run-${bucket}`, triggeredBy: 'automation', status: 'running',
          createdAt: createdAtBuckets[bucket], pluginName: 'video',
        }))
      }
    }
    ...
    // eslint-disable-next-line no-console
    console.log(`[UI-LOGIC] L2-2 关键诊断：逃逸 task 散布在多个 date section（today/yesterday/.../earlier），每个 date section 内有多个 __manual__ 伪 group`)
  })
  ...
  it('L3: filter（plugin / type / status）切换对逃逸的影响', async () => {
    ...
    for (const plugin of plugins) {
      for (let i = 0; i < 5; i++) {
        store.appendTask(makeTask(`t-${idx++}`, {
          runId: `run-${plugin}`, triggeredBy: 'automation', status: i < 3 ? 'completed' : 'running',
          pluginName: plugin,
        }))
      }
    }
  ...
  it('L6: sortBy=activity vs sortBy=created 切换对 flat row / group 内 task 顺序影响 + 逃逸下保稳定', async () => {
    ...
    store.appendTask(makeTask('r1-1', { runId: 'run-1', triggeredBy: 'automation', status: 'completed', createdAt: r1Created, pluginName: 'video' }))
    // patch completedAt 到 r1-1（按 activity 模式它是最新的）
    store.patchTaskById('r1-1', { completedAt: r1Completed } as any)

    store.appendTask(makeTask('r2-1', { runId: 'run-2', triggeredBy: 'automation', status: 'running', createdAt: r2Created, pluginName: 'video' }))

    store.appendTask(makeTask('r3-1', { runId: 'run-3', triggeredBy: 'automation', status: 'queued', createdAt: r3Created, pluginName: 'video' }))
    ...
    // eslint-disable-next-line no-console
    console.log(`[UI-LOGIC] L6-4 sortBy=activity (flat): row 顺序=[${rowKeys.join(', ')}]（按 max 排：r1-1 最近完成 → 排前；r3-1 createdAt 最新 → 排第二；r2-1 排最后）`)
    ...
    const fakeGroups = wrapper.findAll('[data-testid="item-group"]').filter((g) =>
      g.attributes('data-run-id')?.startsWith('__manual__'),
    )
  ...
  it('L7: pinnedRunIds 切换对 group 顺序影响（置顶的 runId 永远在前）', async () => {
    ...
    const now = Date.now()
    store.appendTask(makeTask('a-1', { runId: 'run-1', triggeredBy: 'automation', status: 'running', createdAt: new Date(now - 600 * 1000).toISOString() }))
    store.appendTask(makeTask('a-2', { runId: 'run-2', triggeredBy: 'automation', status: 'running', createdAt: new Date(now - 1200 * 1000).toISOString() }))
    store.appendTask(makeTask('a-3', { runId: 'run-3', triggeredBy: 'automation', status: 'running', createdAt: new Date(now - 1800 * 1000).toISOString() }))
  ...
  it('L8: triggeredByFilter 切换 + 逃逸下保稳定', async () => {
    ...
    for (const r of runs) {
      store.appendTask(makeTask(`t-${idx++}`, {
        runId: r.id, triggeredBy: r.triggeredBy, status: 'running', createdAt: r.createdAt, pluginName: 'video',
      }))
    }
  ...
  it('L9: datePreset 切换（today / 7d / 30d / all / custom）对逃逸的影响', async () => {
    ...
    groups = wrapper.findAll('[data-testid="item-group"]').map((g) => g.attributes('data-run-id'))
    // eslint-disable-next-line no-console
    console.log(`[UI-LOGIC] L9-6 custom=[${thisMonthStart.slice(0, 10)} ~ ${thisMonthEnd.slice(0, 10)}]: 顺序=[${groups.join(', ')}]（期望 run-thisMonth）`)
  ...
  it('L10: 并发切换（viewMode + sortBy + filter + pin + search 同时切）', async () => {
    ...
    for (const r of runs) {
      store.appendTask(makeTask(`t-${idx++}`, {
        runId: r.id, triggeredBy: 'automation',
        status: r.hasCompleted ? 'completed' : 'running',
        createdAt: r.createdAt, pluginName: r.pluginName,
      }))
      if (r.hasCompleted) {
    ...
    let groups = wrapper.findAll('[data-testid="item-group"]').map((g) => g.attributes('data-run-id'))
    // eslint-disable-next-line no-console
    console.log(`[UI-LOGIC] L10-2 切回 group: 顺序=[${groups.join(', ')}]（filter=video 留 r-v1, r-v2; pin r-v2 置顶; search='t-' 全部通过）`)
    ...
    groups = wrapper.findAll('[data-testid="item-group"]').map((g) => g.attributes('data-run-id'))
    // eslint-disable-next-line no-console
    console.log(`[UI-LOGIC] L10-4 清 search/filter/unpin: 顺序=[${groups.join(', ')}]（期望 6 个 run 按 createdAt desc：r-v1, r-v2, r-a1, r-a2, r-i1, r-i2）`)
