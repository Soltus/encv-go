...
describe('clientFilterFiles', () => {
  ...
  it('英文查询 → 大小写不敏感子串匹配', () => {
    const items = [
      makeItem('MyMovie.mp4'),
      makeItem('my-movie.mkv'),
      makeItem('MY-PHOTO.JPG'),
      makeItem('photo.jpg'),
    ]
  ...
  it('带 score 的 item 透传并按 score 微调排序', () => {
    ...
    const items = [
      makeItem('在线视频-低分.mp4', { score: 0.3 }),
      makeItem('在线视频-高分.mp4', { score: 0.9 }),
    ]
    const result = clientFilterFiles(items, '在线视频')
...
describe('clientFilterFiles 性能', () => {
  ...
  it('200 items + CJK 连续查询"在线视频"：应 < 5ms（用户感知 < 100ms）', () => {
    ...
    console.log(`  clientFilterFiles("在线视频") 200 items: avg=${avgMs.toFixed(3)}ms/iter (${ITERATIONS} iters total=${elapsed.toFixed(1)}ms)`)
  })
...
describe('综合加速比验证', () => {
  it('模拟"先搜在线再搜在线视频"场景：第二搜索 < 5ms（vs 冷启动 50ms+，加速 10x+）', () => {
    const items = Array.from({ length: 200 }, (_, i) =>
      makeItem(i < 100 ? `在线播放-视频-${i}.mp4` : `其他-${i}.txt`))
