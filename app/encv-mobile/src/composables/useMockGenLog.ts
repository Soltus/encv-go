...
import type { MockSpecDiag, MockSpecFailed, MockProgress } from '@/api/mockGenerator'
...
export function useMockGenLog(options?: UseMockGenLogOptions) {
  ...
  const mockGenLogSummary = computed<MockGenLogSummary | null>(() => {
    ...
    const disconnected =
      ...
      mockGenLog.value.length < mockGenLogTotal.value &&
      failed + ok < mockGenLogTotal.value
    return {
  ...
  function onSpecDiag(diag: MockSpecDiag): void {
    ...
    const existing = mockGenLog.value.findIndex(
      (e) => e.relativePath === diag.relativePath && e.index === diag.index,
    )
  ...
  function onSpecFailed(fail: MockSpecFailed): void {
    ...
    const e = mockGenLog.value.find(
      (e) => e.relativePath === fail.relativePath && e.status !== 'failed',
    )
    if (e) {
      e.status = 'failed'
