...
describe("useTaskForm - initFromQuery (Files→Tasks 跳转初始化)", () => {
  ...
  it("taskOptions derives from candidates[selectedPluginIndex].taskOptions", () => {
    ...
    candidates.value = [{ name: "video", matchType: "mime", priority: 0, taskOptions: mockOpts }];
    ...
    candidates.value = [
      ...
      },
      {
        ...
        taskOptions: {
          passwordStrategy: "independent" as const,
          supportVersionSelect: false,
          extraFields: [{ key: "plugin_password", label: "PW", type: "password", required: false, defaultValue: "", help: "" }],
        },
      },
    ];
