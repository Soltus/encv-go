...
vi.mock("@/components/VirtualLogList.vue", () => ({
  default: {
    ...
    template:
      '<div class="virtual-log-list-stub"><div v-for="item in items" :key="item.id" class="log-entry" :class="[item.level]"><slot :item="item" /></div></div>',
...
const h = vi.hoisted(() => {
  ...
  const transportConnectionObj = {
    get value() {
      return transportConnectionValue;
    },
    set value(v: any) {
      transportConnectionValue = v;
    },
  };
  return {
...
function createFakeScrollEl(opts: { scrollTop: number; scrollHeight: number; clientHeight: number }): FakeScrollEl {
  ...
  (el as any).__setScrollHeight = (v: number) => {
    state.sh = v;
  };
  (el as any).__setClientHeight = (v: number) => {
    state.ch = v;
  };
...
beforeEach(() => {
  ...
  Object.keys(h.eventBusListeners).forEach(k => {
    h.eventBusListeners[k] = [];
  });
  // 清掉前一轮挂到 body 的 fake 节点
  if (fakeScrollEl?.parentNode) {
...
describe("DevLogs v6：纯手动挡", () => {
  ...
  it("7. activeTab=backend 时 frontend 日志不响应", async () => {
    ...
    await flushPromises();
    h.frontendLogs.value = [...h.frontendLogs.value, { id: 1, timestamp: "00:00:00", level: "info", message: "frontend log" }];
    await nextTick();
    await flushPromises();
  ...
  it("11. rAF coalesce：同帧 5 条 WS 消息合并为 1 次 filter.pushMany", async () => {
    ...
    // 内容验证：每条消息都进了 result
    expect(afterArr.map((e: any) => e.message)).toEqual(["coalesced #0", "coalesced #1", "coalesced #2", "coalesced #3", "coalesced #4"]);
  });
