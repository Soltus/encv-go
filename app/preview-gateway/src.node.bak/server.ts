...
import { ChildrenManager, type ChildSpec } from "./children.js";
import { resolvePaths } from "./paths.js";
import { ensureMockData } from "./preflight.js";
import { ENCV_GO_UPSTREAM, pickUpstream, SPECIAL_UPSTREAMS, type Upstream, VITE_DENY, VITE_UPSTREAM } from "./routing.js";
  ...
  if (up.match === "/openlist-ui") {
    proxy.on("proxyRes", (proxyRes, _req) => {
      ...
      if (ct && /text\/html/i.test(String(ct))) {
        ...
        if (existing) {
          proxyRes.headers["set-cookie"] = Array.isArray(existing) ? [...existing, cookieLine] : [String(existing), cookieLine];
        } else {
...
async function handleHealth(_req: IncomingMessage, res: ServerResponse): Promise<void> {
  ...
  const checks = await Promise.all(upstreamList.map(async up => [up.name, await pingUpstream(up)] as const));
