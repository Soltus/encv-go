...
- `mcp-server.mjs` — Node stdio JSON-RPC server wrapping `./codemogger-shim`.
  ...
  | `codemogger_css_source` | **CSS 产物 → SCSS 源溯源**：给定编译后的 `.css`（须有同目录 `.css.map`，Sass/Vite 产出），解析 source map（base64-VLQ），回答「CSS 第 N 行由哪个 `.scss` partial 的哪一行生成」——连 `@mixin`/`@function`/`@each` 生成的规则（scss 中不以字面量出现）也能精确回源。有 `line`（单行溯源）与无 `line`（按源汇总）两种模式 |
...
### 6. `css-source` — CSS 产物溯源到 SCSS 源

Sass (and Vite's sass pipeline) emit a `*.css.map` source map next to every
...
selector/text never appears literally in the SCSS). `codemogger css-source`
decodes that map (hand-rolled base64-VLQ, no third-party deps) and answers
"which SCSS file/line produced CSS line N?".

```bash
# 按源汇总：每个 .scss 源贡献了多少生成行（含样例映射）
codemogger css-source packages/shared-components/src/theme/.dist/vivid.css

# 单行溯源：CSS 第 52 行由哪个 .scss 的哪一行生成
codemogger css-source packages/shared-components/src/theme/.dist/vivid.css --line 52

# 机器可读
codemogger css-source packages/shared-components/src/theme/.dist/vivid.css --line 52 --json
```

Modes:
- **default (no `--line`)**: per-source summary — how many generated CSS lines
  came from each `.scss` source, with sample line mappings.
- **`--line N`** (1-based): the SCSS provenance of one CSS line
  (`file:line:col` + the originating SCSS snippet).

Source-map discovery: `/*# sourceMappingURL=... */` (path or inline `data:` URI)
in the CSS, else a sibling `<name>.css.map`.

> **Generating a traceable CSS product.** Vite 8 (rolldown) does **not** emit
> `.css.map` from `build.cssSourcemap` in this environment, so a dedicated
> compile step produces the canonical traceable product:
>
> ```bash
> cd packages/shared-components && pnpm build:scss
> # -> src/theme/.dist/{surface,vivid}.css + .css.map  (gitignored)
> ```
>
> Then `codemogger css-source src/theme/.dist/vivid.css` traces the compiled
> rules (incl. the `@each`-generated `--color-*-p3` twins) back to
> `_vivid-p3.scss`. (Runtime still lets Vite compile the `.scss` entry directly;
> this script is only for tracing/verification.)
