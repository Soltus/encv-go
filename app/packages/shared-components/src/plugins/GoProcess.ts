 ...
 *
 * 反模式（禁止）：
 *   import { GoProcess } from '@encv/shared-components/plugins/GoProcess'  // ❌ 未导出
 *   const result = await (GoProcess as any).someMethod()  // ❌ 绕过类型系统
 *
 * 正确模式：
 *   import { someMethod } from '@encv/shared-components/plugins/GoProcess'  // ✅ 用包装函数
...
import type { GoProcessPlugin, GoProcessResult, GoProcessStatus, PermissionCheckResult, PermissionResult, PluginFullState } from "./web";
...
export async function controlOpenList(
  action: "start" | "stop" | "force_db_sync" | "set_admin_password",
