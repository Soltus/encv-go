...
import { getDecodedName, isAlistEncrypted, loadDecodedName, setSessionPassword } from "@encv/shared-components/composables/useAlistEncrypt";
  ...
  if (isAlistEncrypted(file)) {
    return [
        ...
        handler: async (f: FileItem) => {
          try {
            ...
            if (router) {
              router.push({
                path: "/player",
                query: { path: f.path, name: decodedName, alistPath: f.path, alistPassword: password },
              });
            }
          } catch (e) {
      ...
      },
      {
        ...
        handler: async (f: FileItem) => {
          try {
            const { openNewTask } = getAppCapabilities();
