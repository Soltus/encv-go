 ...
 *
 * 📦 已从 encv-mobile 提升进 @encv/shared-components（Phase 4）。
 * 所有依赖（useRunSummaries/useTaskEventBridge/useTaskViewCompute/useWorkflowTaskService/
 * useTaskStore/taskTypeLabel/useI18n/useDateFormat/api）均已在 shared 内。
 */

import { storeToRefs } from "pinia";
import { computed, ref } from "vue";
import type { RunSummary } from "@encv/shared-components/api/encv";
import {
...
import { formatDateTime as dateFormat } from "@encv/shared-components/composables/useDateFormat";
...
import { useTaskViewCompute } from "@encv/shared-components/composables/useTaskViewCompute";
import { useWorkflowTaskService } from "@encv/shared-components/composables/useWorkflowTaskService";
import { getTaskTypeLabel } from "@encv/shared-components/lib/taskTypeLabel";
import { useTaskStore } from "@encv/shared-components/stores/taskStore";
import { computeGroupCounters, getTaskDisplayName } from "@encv/shared-components/lib/taskViewComputeCore";
...
// ============ helper：任务显示 ============
// getTaskDisplayName 已从 @encv/shared-components/lib/taskViewComputeCore 导入（单一真源）
...
// ============ helper：group 派生（委托 core 单一真源 computeGroupCounters） ============
function createUseTasksList() {
  ...
  return {
    ...
    // 内部：给 group-detail 使用（委托 core 单一真源）
    _computeGroupCounters: (tasks: EncvTask[]) =>
      computeGroupCounters(tasks, {
        searchQuery: storeRefs.searchQuery.value,
        filterPlugins: storeRefs.filterPlugins.value,
        filterTypes: storeRefs.filterTypes.value,
        filterStatuses: storeRefs.filterStatuses.value,
        filterDateRange: storeRefs.filterDateRange.value,
      }),
    workflowService,
  };
}
