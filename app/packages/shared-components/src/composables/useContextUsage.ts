...
export function useContextUsage(opts: UseContextUsageOptions) {
  ...
  async function fetchOnce() {
    ...
    loading.value = true;
    try {
      // 注意：Agent API 走 getAgentApiBase()（dev=preview-gateway /agent-api、
      // APK=相对路径由 ApiProxy 接管），与 apiRequest 的标准后端 base 不同，
      // 故此处保留裸 fetch（与 A 批「有意不收敛」的探针/特殊 base 同类）。
  ...
  // 轮询样板（interval 自调度 + 并发守卫 + onUnmounted 清理 + 事件重探）统一交给 usePoll
  const poll = usePoll(fetchOnce, {
