...
import type { TestModule, WebDavTestContext } from "@encv/shared-components/types/webdav-test";
...
function hasVirtualFile(ctx: WebDavTestContext, virtualPath: string): boolean {
  return ctx.activeMount.manifest.virtual_tree.some(e => e.virtual_path === virtualPath && !e.is_dir);
}
  ...
  cases: [
    {
      ...
      url: ctx => {
        ...
        const largest = ctx.activeMount.manifest.virtual_tree.filter(e => !e.is_dir).sort((a, b) => b.size - a.size)[0];
    ...
    },
    {
      ...
      expect: {
        ...
        bodyNotMatches: ctx => {
          ...
          return new RegExp(`<href[^>]*>[^<]*${escaped}(?:[^<]|\\s)*</href>`, "i");
        },
      },
      skip: ctx => !pickFirstContainer(ctx),
      order: 3,
  ...
  cases: [
    ...
    },
    {
      ...
      url: ctx => {
        const container = ctx.activeMount.manifest.container_map[0];
        if (!container) return joinUrl(ctx);
        const files = ctx.activeMount.manifest.virtual_tree.filter(
      ...
      url: ctx => {
        ...
        try {
          ...
          const target = new URL(
            other.webdav_path.replace(/^\//, "") + "/" + container.virtual_path.replace(/^\//, ""),
