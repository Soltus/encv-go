 ...
 *
 * 依赖注入：listRuns / getRunSummary 来自应用层 api（@/api/encv_tasks），
 * 共享层无法自行实现（依赖应用层 base URL），故通过 TaskServices 注入，
 * 由 encv-mobile 在启动时 setTaskServices 注入真实实现。
 */

import { type ComputedRef, computed, type Ref, ref } from "vue";
import { getTaskServices } from "@encv/shared-components/stores/taskServices";
import type { RunInfo, RunSummary } from "@encv/shared-components/types/task";
...
export function useRunSummaries(): UseRunSummaries {
  ...
  async function fetchAll(): Promise<void> {
    ...
    try {
      const list = await getTaskServices().listRuns();
      runs.value = list;
  ...
  async function fetchOne(runId: string): Promise<void> {
    if (!runId) return;
    try {
      const summary = await getTaskServices().getRunSummary(runId);
...
/** 模块级单例（跨组件共享 run summary 数据），经 defineSingleton 收敛样板 */
const _runSummaries = defineSingleton(useRunSummaries);

export function useRunSummariesSingleton(): UseRunSummaries {
  return _runSummaries.get();
}
