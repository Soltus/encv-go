...
import { cancelTask } from "@encv/shared-components/api/encv_tasks";
import { enqueueCancelWorker, isNative } from "@encv/shared-components/runtime/nativeBridge";
...
export function useTaskCancel() {
  ...
  async function pollCancelStatus(taskId: string, maxAttempts = 10, intervalMs = 2000): Promise<boolean> {
    for (let i = 0; i < maxAttempts; i++) {
      try {
        // 走统一 apiRequest（标准后端 base + 错误归一化），替代手写 fetch + ok/json
        const data = await apiRequest<{ status?: string }>(`/api/tasks/${encodeURIComponent(taskId)}`);
