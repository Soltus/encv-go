 ...
 *
 * 🆕 提升说明（Phase 3 useTasksList 依赖簇）：
 * 原应用层依赖（batchCreateTasks / cancelRun / useTaskStore.appendTask / analyzeError）
 * 通过 TaskServices DI 或共享模块提供，本文件不再直接 import 应用层实现。
 */
...
import { expandMatrix, isMatrixStrategy } from "@encv/shared-components/lib/workflow/matrixExpander";
...
import type { BatchTaskSpec, EncvTask } from "@encv/shared-components/types/task";
import { applyTerminalGuard, computeJobConclusion, inferWorkflowStatus, validateTransition } from "../lib/workflow/state-machine";
import {
  type ActionSpec,
...
function createService(options: WorkflowTaskServiceOptions): WorkflowTaskService {
  ...
  async function executeJob(jobDef: JobDefinition, env: Record<string, string>, jobRun: JobRun, runId: string): Promise<JobRun> {
    ...
    if (executableSteps.length > 0) {
      ...
      // 2. 一次性批量提交（后端生成 UUID）
      try {
        const tasks: EncvTask[] = await getTaskServices().batchCreateTasks(batchSpecs, runId, taskTriggeredBy);

        // 3. 一次性 push 所有真实 task 到 store（UI 立即显示 1 个 group N task）
        const { appendTask } = getTaskServices();
  ...
  async function cancelRun(runId: string): Promise<void> {
    ...
    try {
      await getTaskServices().cancelRun(runId);
    } catch (e) {
