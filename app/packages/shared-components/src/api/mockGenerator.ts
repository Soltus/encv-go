...
import type { MockFileType } from "@encv/shared-components/lib/mockDataGenerator";
...
export async function generateMockFilesViaBackend(opts: MockGenerateOptions): Promise<MockGenerateResult> {
  ...
  // 🆕 抽取 SSE 事件分发，供主循环与流结束 flush 共用（避免重复逻辑）
  const dispatch = (parsed: { event: string; data: string }): void => {
