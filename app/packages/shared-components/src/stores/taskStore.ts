/**
 * taskStore — 唯一任务数据 + 视图状态 owner（共享抽象层）
 *
 ...
 *
 * 依赖注入：本 store 不感知应用层实现，所有应用层能力（向量搜索 / IndexedDB 持久化）
 * 通过 ./taskServices 注入，由应用层在启动时 setTaskServices。
 */
...
// 见 encv-mobile 原注释：MAX_LOADED_TASKS=10000 用于 WS 视图分页保护（非 100）
export const MAX_LOADED_TASKS = 10000;

export const useTaskStore = defineStore("task", () => {
  // 应用层服务（启动时已注入；setup 在首次 useTaskStore 时执行，晚于注册）
  ...
  async function hydrate(): Promise<void> {
    ...
    try {
      const list = await persistence.loadAllTasks();
      tasks.value = list;
      hasAnyTask.value = list.length > 0;
      rebuildIndex();
      hydrated.value = true;
      void persistence.ensureLRUCache();
    } catch (_err) {
      hydrated.value = true;
    }
  }
  ...
  function persistRemove(id: string): void {
    try {
      persistence.clearPutThrottle(id);
      void persistence.deleteTask(id);
    } catch {}
  }
  ...
  function bulkSetTasks(newTasks: EncvTask[]): void {
    ...
    if (hydrated.value) {
      try {
        void persistence.bulkPutTasks(merged);
      } catch {}
    }
  }
  ...
  function appendTasksPage(newTasks: EncvTask[]): void {
    ...
    if (hydrated.value) {
      try {
        void persistence.bulkPutTasks(toAdd);
      } catch {}
    }
  }
  ...
  function rebuildFromBackend(serverTasks: EncvTask[]): void {
    ...
    if (changed) {
      ...
      if (hydrated.value) {
        try {
          void persistence.bulkPutTasks(serverTasks);
        } catch {}
      }
    }
  }
