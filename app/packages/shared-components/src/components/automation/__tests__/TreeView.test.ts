...
describe("TreeView - 基础渲染", () => {
  ...
  it("渲染多个父节点", () => {
    const wrapper = mountTree({
      nodes: [makeNode({ id: "job-1", label: "Job A" }), makeNode({ id: "job-2", label: "Job B" })],
    });
    const parentNodes = wrapper.findAll(".tree__node--parent");
...
describe("TreeView - 搜索过滤", () => {
  ...
  it("按父节点 label 过滤", async () => {
    const wrapper = mountTree({
      nodes: [makeNode({ id: "job-1", label: "Encrypt All" }), makeNode({ id: "job-2", label: "Decrypt All" })],
    });
  ...
  it("tree__count 显示过滤后节点数", () => {
    const wrapper = mountTree({
      nodes: [makeNode({ id: "job-1" }), makeNode({ id: "job-2" })],
    });
    ...
    const wrapper = mountTree({
      nodes: [
        makeNode({
          id: "job-ok",
          label: "All Success",
          children: [{ id: "s1", label: "step1", status: "success" }],
        }),
        makeNode({
          id: "job-fail",
          label: "Has Failure",
    ...
    const wrapper = mountTree({
      nodes: [
        makeNode({
          id: "job-timeout",
          label: "Has Timeout",
          children: [{ id: "s1", label: "step1", status: "timed_out" }],
        }),
      ],
    });
    ...
    const wrapper = mountTree({
      ...
      nodes: [
        makeNode({
          id: "job-fail",
          label: "Has Failure",
          children: [{ id: "s1", label: "step1", status: "failure" }],
        }),
      ],
    });
...
describe("TreeView - 进度/耗时/速率字段显示", () => {
  ...
  it("子节点的 progress 字段渲染", async () => {
    const wrapper = mountTree({
      nodes: [
        makeNode({
          children: [{ id: "s1", label: "step1", status: "running", progress: 50 }],
        }),
      ],
    });
    await wrapper.find(".tree__node--parent").trigger("click");
...
describe("TreeView - Phase 图标集成", () => {
  ...
  it("子节点 phase 存在时渲染 PhaseIcon", async () => {
    const wrapper = mountTree({
      nodes: [
        makeNode({
          children: [{ id: "s1", label: "step1", status: "running", phase: Phase.Packing }],
        }),
      ],
    });
    await wrapper.find(".tree__node--parent").trigger("click");
...
describe("TreeView - 状态色 class", () => {
  ...
  it.each(statusCases)("父节点 status=%s 时包含 class %s", (status, expectedClass) => {
    const wrapper = mountTree({
