...
async function freshModules() {
  ...
  const GroupDetail = (await import("@encv/shared-components/views/GroupDetail.vue")).default;
  const { useRunTasksStoreSingleton, __resetRunTasksStoreForTests } = await import("@encv/shared-components/stores/runTasksStore");
  const { useRunSummariesSingleton, __resetRunSummariesForTests } = await import("@encv/shared-components/composables/useRunSummaries");
...
async function mountGroupDetail(opts: { runId?: string; tasksFixture?: EncvTask[]; summaryFixture?: RunSummary } = {}) {
  ...
  const { getTasks, getRunSummary } = await import("@encv/shared-components/api/encv");
  ...
  const { useWorkflowTaskService } = await import("@encv/shared-components/composables/useWorkflowTaskService");
...
describe("GroupDetail.vue 真机组件测试", () => {
  ...
  it("独立加载：进入时调 GET /api/tasks?runId=xxx 加载该 run 的 task", async () => {
    ...
    const { getTasks } = await import("@encv/shared-components/api/encv");
  ...
  it("WS task:completed 后刷新对应 runId 的 summary", async () => {
    ...
    const { getRunSummary } = await import("@encv/shared-components/api/encv");
