...
import { actionSheetController, menuController, onIonViewWillEnter } from "@ionic/vue";
...
import { eventBus, useEventBusListener } from "@encv/shared-components/composables/useEventBus";
...
import { convertBooleanQueryToVectorKeywords } from "@encv/shared-components/lib/searchQuery";

function showErrorToast(message: string): void {
  showToast({ message, duration: 2000, color: "danger" });
}
...
export function useFilesView(): UseFilesViewReturn {
  ...
  async function playMedia(file: FileItem, category: string) {
    ...
    switch (mode) {
      ...
      case PLAY_MODE.MPV_COMPOSE:
        if (getAppCapabilities().isNative()) {
          const result = await getAppCapabilities().openPlayer(file.path, file.name, mimeType, mode);
          if (!result.success) {
      ...
      case PLAY_MODE.EXTERNAL:
        if (getAppCapabilities().isNative()) {
          ...
          getAppCapabilities().openExternal(url, mimeType);
        } else {
          router.push({ path: "/player", query: { path: file.path, name: file.name } });
        }
        break;
  ...
  const MAX_RETRIES = getAppCapabilities().isNative() ? 15 : 3;
  ...
  async function handleRequestStorage() {
    ...
    await getAppCapabilities().requestStoragePermission();
  ...
  async function handleRename(file: FileItem) {
    ...
    try {
      ...
      showErrorToast(err.message || "Rename failed");
    }
  }
  ...
  async function handleMove(file: FileItem) {
    ...
    try {
      ...
      showErrorToast(err.message || "Move failed");
    }
  }
  ...
  async function handleShare(file: FileItem) {
    if (getAppCapabilities().isNative()) {
      try {
        const localPath = await getAppCapabilities().getLocalFilePath(file.path);
        if (localPath) {
  ...
  async function handleDeleteFile(file: FileItem) {
    ...
    if (file.isDirectory) {
      ...
      const ok = await confirm({
        ...
        subHeader: file.isDirectory ? `📁 ${file.name}` : undefined,
        message: file.isDirectory
          ? `确认删除文件夹 "${file.name}" 及其所有内容？\n\n${detail}`
          : t("files.deleteConfirm", { name: file.name }),
        danger: true,
      });
      if (ok) doDelete(file);
    }
  ...
  // file:change 订阅随组件卸载自动清理（K17 useEventBusListener）
  useEventBusListener("file:change", onFileChange);
  ...
  onMounted(() => {
    ...
    if (import.meta.env.DEV) {
      getAppCapabilities().registerTestBackdoor?.({
        files,
