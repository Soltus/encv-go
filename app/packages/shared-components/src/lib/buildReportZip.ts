...
import { formatBytes, truncatePath, truncateText } from "./format";
import { formatDuration } from "@encv/shared-components/composables/useDateFormat";
...
function buildPerformanceMd(
  ...
  for (const pluginName of Object.keys(perfByPlugin)) {
    ...
    for (const c of pluginCases) {
      ...
      lines.push(
        `| ${String(c.index).padStart(3, "0")} | ${p.avgThroughput.toFixed(1)} MB/s | ${gradeLabel} (${p.gradeScore.toFixed(0)}) | ${formatDuration(p.totalDurationMs, { showMs: true, showDecimals: true, invalid: "N/A" })} | ${formatBytes(p.sourceSize)} | ${formatBytes(p.outputSize)} |`
      );
    }
    lines.push("");
  }
...
function buildCaseDetailMd(c: CaseDetail, caseNumber: number, t: TFunction): string {
  ...
  if (c.performance) {
    ...
    lines.push(
      `| ${t("tasks.performance.totalDuration")} | ${formatDuration(p.totalDurationMs, { showMs: true, showDecimals: true, invalid: "N/A" })} |`
    );
...
/** 截断错误（取首行后保留开头，省略符 "..."，行为与原实现一致） */
function truncateError(e: string, max = 120): string {
  if (!e) return "";
  const firstLine = e.split("\n")[0];
  return truncateText(firstLine, max, "...", "head");
}
