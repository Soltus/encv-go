...
vi.mock("@/api/encv", async () => {
  ...
  return {
    ...actual,
    batchCreateTasks: vi.fn(async (specs: any[], runId?: string, triggeredBy?: string): Promise<EncvTask[]> => {
...
import { buildDynamicWorkflowPure } from "@encv/shared-components/lib/workflow/buildDynamicWorkflow";
...
describe('真机"任务逃逸"e2e — 批量创建 task（真实架构实现）', () => {
  ...
  it('T4: submitRun 后，1:1 复刻 UI 立即显示 1 个真 group N task（不再"慢慢累加"）', async () => {
    ...
    // eslint-disable-next-line no-console
    console.log(
      `[T4] submitRun 后: totalTask=${totalTaskCount} / 真 group=${realGroupCount} / 伪 group=${fakeGroupCount} / 逃逸=${escapeTaskCount}`
    );
  ...
  it("T5: WS task:created 广播带 runId——store 中所有 task runId 非空 + appendTask 不触发 warn", async () => {
    ...
    const appendTaskWarns = warnSpy.mock.calls.filter(args => typeof args[0] === "string" && args[0].includes("[taskStore.appendTask]"));
