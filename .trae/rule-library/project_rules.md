...
> 本文件为 [project_rules.md](../rules/project_rules.md) 的详情文档（包含完整代码、反模式实战、`.gitignore` 清单、验证脚本等）。
>
> **返回索引**：[rules/project_rules.md](../rules/project_rules.md)

---

## 一、Trae Web 沙箱网络限制（本地构建必读）

> 来自实战踩坑：Java/JVM 进程**完全无法**在沙箱内出站 TCP 连接（任何 JDK 版本都受影响）。

- **沙箱禁止 Java/JVM 进程出站 TCP 连接**（所有 JDK 版本均受影响）
- **CI 环境不受此限制**，Gradle 构建应在 CI 执行
- 沙箱内 `curl`/`npm`(走 MCP HTTP 代理) 可正常联网
- 进程级策略矩阵 + 完整诊断数据 → [trae_web_sandbox_network.md（索引）](../rules/trae_web_sandbox_network.md) + [rule-library/trae_web_sandbox_network.md（详情）](../rule-library/trae_web_sandbox_network.md)
...
- 相关文件：
  - `internal/utils/ffmpeg_dlopen.go`（Android）
  - `internal/utils/ffmpeg_dlopen_stub.go`（桌面端）
  - `internal/utils/video.go`
  - `internal/utils/build_info.go`
...
### 7.2 Gradle/Maven 镜像（国内网络）

> 当沙箱无法访问 `maven.google.com` / `repo.maven.org` 时，使用以下镜像（来自 Sillot-KMP settings.gradle.kts）：

#### 7.2.1 pluginManagement.repositories 镜像顺序
...
#### 7.2.2 dependencyResolutionManagement.repositories 镜像顺序
...
> ⚠️ 镜像顺序的硬性约束 + 阿里云不能代理 Gradle Plugin Portal 的根因 → [android.md（索引）](../rules/android.md) + [rule-library/android.md（详情）](../rule-library/android.md)
...
## 九、Jetpack Compose 编码规范

- **权威参照文件**：[compose-reference.md](../rules/compose-reference.md)（Android 官方文档摘录 + 本项目已验证代码）
...
### 10.2 二、不确定时阻断，不猜测（Fail-Safe 原则）
