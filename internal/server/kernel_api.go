...
func (s *Server) handleKernelMkStatsGin(c *gin.Context) {
	...
	stats := gin.H{
		...
		"taskRecorderEnabled": mk.TaskRecorder() != nil && mk.TaskRecorder().Enabled(),
	}
	...
	if mk.TaskRecorder() != nil && mk.TaskRecorder().Enabled() {
		if taskStats, err := mk.TaskRecorder().Store().Stats(kernel.TaskRecordFilter{}); err == nil {
...
// handleKernelMkTasksListGin 查询任务列表
//
// Query 参数：
//   - type: 任务类型过滤（可多个，逗号分隔）
//   - service: 服务名过滤（可多个，逗号分隔）
//   - status: 状态过滤（可多个，逗号分隔）
//   - tenant: 租户 ID
//   - triggeredBy: 触发者
...
//   - offset: 偏移量
//   - sortBy: 排序字段（createdAt / updatedAt / durationMs）
//   - sortOrder: 排序方向（asc / desc，默认 desc）
func (s *Server) handleKernelMkTasksListGin(c *gin.Context) {
	mk := s.kernelMicroOr503(c)
	if mk == nil {
		return
	}
	...
	if tr == nil || !tr.Enabled() {
		c.JSON(http.StatusNotImplemented, gin.H{
			"ok":    false,
			"error": "task recorder not enabled",
		})
		return
	}

	filter := kernel.TaskRecordFilter{}

	// 类型过滤
	if v := c.Query("type"); v != "" {
		types := strings.Split(v, ",")
		for _, t := range types {
			filter.Types = append(filter.Types, kernel.TaskType(t))
		}
	}
	// 服务过滤
	if v := c.Query("service"); v != "" {
		filter.Services = strings.Split(v, ",")
	}
	// 状态过滤
	if v := c.Query("status"); v != "" {
		statuses := strings.Split(v, ",")
		for _, st := range statuses {
			filter.Statuses = append(filter.Statuses, kernel.TaskStatus(st))
		}
	}
	// 租户
	...
	tasks, err := tr.Store().List(filter)
	if err != nil {
		c.JSON(http.StatusInternalServerError, gin.H{
			"ok":    false,
			"error": err.Error(),
...
// handleKernelMkTasksGetGin 获取单条任务详情
func (s *Server) handleKernelMkTasksGetGin(c *gin.Context) {
	mk := s.kernelMicroOr503(c)
	if mk == nil {
		return
	}
...
// handleKernelMkTasksStatsGin 任务统计
func (s *Server) handleKernelMkTasksStatsGin(c *gin.Context) {
	mk := s.kernelMicroOr503(c)
	if mk == nil {
		return
	}
	...
	stats, err := tr.Store().Stats(filter)
	if err != nil {
		c.JSON(http.StatusInternalServerError, gin.H{
			"ok":    false,
			"error": err.Error(),
